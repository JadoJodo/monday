...
func newInstallCmd(gf *globalFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Generates a LaunchAgent that runs monday daily; monday itself " +
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			// `monday run`, which refuses to execute without one. This also
			...
			// an agent: the plist runs `monday run`, which would fail every fire on a
			...
			// The agent runs daily and lets monday's schedule decide which
